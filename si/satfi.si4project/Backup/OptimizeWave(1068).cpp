...
double *xd_realloc(double *ptr, unsigned nitems)
{
	double *tmp;
...
void *xx_realloc(char *ptr, unsigned size)
{
	void *tmp;
	if (ptr == NULL)
...
void multirr(int length, short *id, double *win, double *frame)
{
...
int OptimizeWave(char *strSrcFile)
{
	short *is, *ix;
	int  m0, i, j;
	int  length = 0;
