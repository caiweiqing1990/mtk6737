	...
	while (1)
    {
						...
						if(memcmp(clientAddr1 ,&clientAddr, len) == 0)
						{
							...
							else
							{
								if(base->sat.sat_state_phone == SAT_STATE_PHONE_ONLINE)
								{
									...
									{
										static unsigned short *testi=(unsigned short *)tmp;
										static unsigned short testj=0;
										if(start == 0)
										{
		...
		while(base->sat.secondLinePhoneMode == 1)
		{
			...
			if(base->sat.sat_state_phone == SAT_STATE_PHONE_ONLINE)
			{
				...
				static unsigned short *testi=(unsigned short *)echo;
				static unsigned short testj=0;
				...
				//for(i=0;i<3;i++)
				{
					if(aecmInst)
					{
						WebRtcAec_Process(aecmInst, (short*)&tmp[i*320], NULL, (short*)echo, NULL, NN, 120, 0);
					}
			...
			default:
				...
				if(base.sat.sat_pcmdata > 0 && FD_ISSET(base.sat.sat_pcmdata, &fds)) {
					...
					if(n > 0)
					{
						...
						{
							//satfi_log("for test n = %d\n", n);
							write(base.sat.sat_pcmdata, SatDataBuf[3], 320);
							if(start == 0)
							{
								clock_gettime(CLOCK_REALTIME, &time_start);
								start = time_start.tv_sec*1000000000 + time_start.tv_nsec;
