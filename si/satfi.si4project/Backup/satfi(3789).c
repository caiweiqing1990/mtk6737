			...
			else if(idx>4)
			{
				...
				else if(strstr(data,"+CMT"))
				{
					...
					{					
						...
						satfi_log("MessageParse %s %s %d %d %d\n", OAphonenum, Decode, *satfd, i, strlen(data));
						char tmp[1024]={0};
...
int handle_app_msg_tcp(int socket, char *pack, char *tscbuf)
{
	...
	switch(p->mclass)
	{
		...
		case SOS_INFO:
		{
			...
			SetKeyString("SOS", "PHONE", SOS_FILE, NULL, Message);
		...
		if(pin_stat != stat)
		{
			...
			if(pin_stat == 0)
			{
				if(isFileExists(SOS_FILE))
				{
					...
					satfi_log("MESSAGE:%s\n", ucTmp);
					strncpy(toPhone, ucTmp, sizeof(messagedata));
...
static void *sendto_app_voice_udp(void *p)
{
	...
	while(1)
	{
		if(base->sat.sat_calling == 1)
		{
			...
			{
				int iRecordCH=0;
				myexec("insmod /storage/self/primary/pcm_master_gpio.ko", NULL, NULL);				
		...
		else
		{
			if(pcmfd>0)
			{
				...
				myexec("rmmod /storage/self/primary/pcm_master_gpio.ko", NULL, NULL);
			}
			sleep(1);
		}
	}
