...
void *handle_pcm_data(void *p)
{
	...
	while (1)
    {
		...
		//{
		//	if(base->sat.sat_state_phone == SAT_STATE_PHONE_ONLINE && base->sat.locak_socket_audio_cancel <= 0)
		//	{
						...
						else
						{
							...
							if(base.sat.secondLinePhoneMode == 1)
							{
								...
								//if(base.sat.sat_state_phone == SAT_STATE_PHONE_ONLINE)
								{
									...
									{
										write(base.sat.locak_socket_audio_cancel, SatDataBuf[3], n);
									}
								}
	...
	while(1)
	{
		...
		else
		{
			...
			if(base->sat.locak_socket_audio_cancel > 0)
			{
				...
				{
					int n = read(base->sat.locak_socket_audio_cancel, buf+ofs, 320);
					if(n>0)
					{
						ofs += n;
						if(ofs >= 960)
						{
							...
							{
								//satfi_log("%d", ofs1);
								write(base->sat.sat_pcmdata, buf+ofs1, n);
