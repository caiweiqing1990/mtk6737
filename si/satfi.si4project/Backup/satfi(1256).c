		...
		if(base->sat.sat_fd > 0 && base->sat.sat_calling == 0)
		{
			...
			switch(base->sat.sat_state)
			{
				...
				case SAT_STATE_CGACT_SCCUSS:
					...
					//uart_send(base->sat.sat_fd, "AT\r\n", 4);
					uart_send(base->sat.sat_phone, "AT\r\n", 4);
					if(base->sat.sat_msg_sending == 0)uart_send(base->sat.sat_message, "AT\r\n", 4);
					break;
				default:
			...
			else if(idx>4)
			{
				...
				{	
					...
					if(idx>18 && data[idx-2]=='\n' && data[idx-1]=='\n')
					{
						...
						strncpy(OAphonenum, "13112121509", 21);
						strncpy(Decode, "13112121509", 21);
						satfi_log("MessageParse %s %s %d\n", OAphonenum, Decode, strlen(&data[i+2]));
...
static void *CallUpThread(void *p)
{
	...
	//if(base->sat.sat_status == 1)base->sat.sat_state = SAT_STATE_CGACT;
	//sleep(5);
	...
	while (1)
    {
		...
		switch(select(maxfd+1,&fds,NULL,NULL,&tout))
		{
			...
			default:
				...
				{
			        n = recvfrom(sock, &tmp[ofs], BUS_SIZE>>1, 0, (struct sockaddr*)&clientAddr, &len);
...
int main(void)
{
	...
	//char Decode[1024]= {0};
	//MessageParse("0891683108200455F32405A10180F6000891304251829223440077006500690069007700650069558255827EFF82727684549655615473963F84285FB753D165AF848282AC963F84285FB76CD55E086CD5573065B9963F65AF848282AC", OAphonenum, Decode);
