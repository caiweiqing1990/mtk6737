...
/* 3p6s */
#define PRINTKR  0  // or define to 0, to disable the print 
#define PRINTKT  0  // or define to 0, to disable the print 
...
static u8 *tx_buf; //This needs to be DMA friendly buffer 
...
struct mt_chip_conf spi_fifo_conf_mt65xx = {
	.setuptime =15,
	.holdtime = 15,
	.high_time = 10,  //   ¨®?low_time  12¨ª????¡§¡Á?SPI CLK ¦Ì??¨¹?¨²
	.low_time = 10,
	.cs_idletime = 20,
	
	.rx_mlsb = 1,       
	.tx_mlsb = 1,  
	//mlsb=1  ¡À¨ª¨º???bit???¨¨¡ä?¡ê?¨ª¡§3¡ê2?D¨¨¨°a??
	.tx_endian = 0,     //tx_endian  =1 ¡À¨ª¨º?¡ä¨®???¡ê¨º?¡ê???¨®¨²DMA?¡ê¨º?¡ê?D¨¨¨°a?¨´?Y¨¦¨¨¡À?¦Ì?Spec ¨¤¡ä????¡ê?¨ª¡§3¡ê?a¡ä¨®???¡ê
	.rx_endian = 0,
	
	.cpol = 0,      //  ?a¨¤?2?D¨¨¨°a?¨´¨¦¨¨?? ¡ê?  xxxx_spi_client->mode = SPI_MODE_0 ?a¨¤?¨¦¨¨???¡ä?¨¦
	.cpha = 0,     //   ?a¨¤?2?D¨¨¨°a?¨´¨¦¨¨?? ¡ê?  xxxx_spi_client->mode = SPI_MODE_0 ?a¨¤?¨¦¨¨???¡ä?¨¦
	
	//spi_par->com_mod = DMA_TRANSFER;     // DMA  or FIFO
	.com_mod = FIFO_TRANSFER,
	.pause = 0,     //¨®? deassert  ¦Ì?¨°a???¨¤¡¤¡ä?¡ê?¡ä¨º?¡¤??¡ì3??Y¨ª¡ê?¡ê¨º?¡ê?¨¨?¡ä?¡Á?SPI_CS ?¨²?¨¤¡ä?transfer???? 2??¨¢¡À?de-active
...
#define DM9051_PWR_2_PIN (GPIO94 | 0x80000000)  //(GPIO91 | 0x80000000)
#define DM9051_PWR  (GPIO94 | 0x80000000)

int SPI_GPIO_Set(int enable)
{
	mt_set_gpio_mode(DM9051_PWR, 0);
	mt_set_gpio_out(DM9051_PWR, 1);//(DM9051_PWR,1);
	
	mt_set_gpio_mode(DM9051_PWR_2_PIN, 0);
	mt_set_gpio_out(DM9051_PWR_2_PIN, 1);//(DM9051_PWR,1);
	...
	mt_set_gpio_mode(GPIO_SPI_MISO_PIN, 2);
	...
	mt_set_gpio_mode(GPIO_SPI_MOSI_PIN, 2);
	...
	*/
			printk("CMMB GPIO CS SPI PIN mode:num:%d, %d,out:%d, dir:%d,pullen:%d,pullup%d",GPIO_SPI_CS_PIN,mt_get_gpio_mode(GPIO_SPI_CS_PIN),mt_get_gpio_out(GPIO_SPI_CS_PIN),
...
static int SUBLCD_SPI_Init(struct board_info *db, int enable)
{
	...
	if(enable){
		...
		spi_par->high_time = 4; // 2      //10--6m   15--4m   20--3m  30--2m  [ 60--1m 120--0.5m  300--0.2m]
		spi_par->low_time = 4;  //2
...
#ifdef CONFIG_HAS_EARLYSUSPEND
static void dm9000_drv_early_suspend(struct early_suspend *h)
{    
	//struct  *gfx1xm_dev = container_of(h, struct gfx1xm_dev, early_fp);

	struct board_info *db = container_of(h,struct board_info,early_fp);
    struct net_device *ndev = db->ndev;
	printk("DM9051 >>>>>>>>>>dm9000_drv_early_suspend \n");

	if (ndev) 
	{
		db->in_suspend = 1;
		printk("dm9000_drv_early_suspend ..2 \n");
		if (netif_running(ndev)) 
		{
			...
			mt_set_gpio_mode(DM9051_PWR, 0);
			mt_set_gpio_out(DM9051_PWR, 0);//(DM9051_PWR,1);
			
			mt_set_gpio_mode(DM9051_PWR_2_PIN, 0);
			mt_set_gpio_out(DM9051_PWR_2_PIN, 0);//(DM9051_PWR,1);
	    }
	}
	
}
	/*----------------------------------------------------------------------------*/
static void dm9000_drv_late_resume(struct early_suspend *h)
{    
	struct board_info *db = container_of(h,struct board_info,early_fp);
	struct net_device *ndev = db->ndev;
	printk("DM9051 >>>>>>>>>>dm9000_drv_late_resume \n");
	...
	if (ndev) 
	{
		if (netif_running(ndev)) 
		{
			...
			printk("dm9000_drv_late_resume ..2 \n");
			dm9051_open(ndev);
...
//#if DMA3_P2_MSEL_MOD
	...
	#ifdef CONFIG_HAS_EARLYSUSPEND
	db->early_fp.level		= EARLY_SUSPEND_LEVEL_DISABLE_FB - 1,
	db->early_fp.resume = dm9000_drv_late_resume;
	db->early_fp.suspend = dm9000_drv_early_suspend;
	
	register_early_suspend(&db->early_fp);
	#endif 
...
static int /* __init */ dm9051_init(void)
{
	printk("\n");
	printk("%s Driver\n",CARDNAME_9051);
	printk("%s Driver, V%s (%s)\n", CARDNAME_9051,DRV_VERSION, str_drv_xmit_type);
