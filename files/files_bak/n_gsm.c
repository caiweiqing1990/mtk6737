...
static void gsmld_write_wakeup_tsk(unsigned long arg);
...
static void gsm_data_kick(struct gsm_mux *gsm)
{
	...
	list_for_each_entry_safe(msg, nmsg, &gsm->tx_list, list) {
		...
		if (debug & 4)
			print_hex_dump_bytes("gsmld_output: ",
					     DUMP_PREFIX_OFFSET,
					     gsm->txframe, len);
							...
							u8 *data, int clen)
{
	...
	case CMD_PSC:
		...
		break;
	case CMD_SNC: {
		int initiator = gsm->initiator;
		gsm->initiator = 0;
		gsm->ftype = gsm->control;
		//printk("gsmld %d %02x\n", initiator, gsm->ftype);
		gsm_control_reply(gsm, CMD_SNC, data, clen);
		gsm->initiator = initiator;
		}
		break;
...
static struct gsm_dlci *gsm_dlci_alloc(struct gsm_mux *gsm, int addr)
{
	...
	if (kfifo_alloc(&dlci->_fifo, 40960 *2, GFP_KERNEL) < 0) {
		kfree(dlci);
		return NULL;
	}
...
static int gsm_activate_mux(struct gsm_mux *gsm)
{
    ...
    tasklet_init(&gsm->tsk, gsmld_write_wakeup_tsk, (unsigned long) gsm);
...
static int __init gsm_init(void)
{
	...
	gsm_tty_driver = alloc_tty_driver(256); //动态分配256个tty驱动
	if (!gsm_tty_driver) {
		tty_unregister_ldisc(N_GSM0710);
		pr_err("gsm_init: tty allocation failed.\n");
		return -EINVAL;
	}
	gsm_tty_driver->driver_name	= "gsmtty";  //绑定驱动的名称为gsmtty
	...
	if (tty_register_driver(gsm_tty_driver)) {  //注册gsm_tty_driver驱动
